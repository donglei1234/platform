...
// MatchKeyValue packs up a keys/value search pair when scanning for a db.
func MatchKeyValue(idx string, val string) ScanOption {
	return func(o *ScanOptions) error {
		if idx == "" {
			return errors.Wrap(keys.ErrInvalidKeyFormat, "empty keys provided")
		} else {
			o.Query = append(o.Query, ScanQuery{
...
// MatchKeyLike packs up a keys/value search pair when scanning for a db - for a sql style "like" match.
