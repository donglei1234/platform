...
type GrpcServerFactory func(logger *zap.Logger, listener HasGrpcListener, port Port, opts ...grpc.ServerOption) (GrpcServer, error)
...
func createNewTestTcpServer(t *testing.T) *TcpGrpcServer {
	listener := GrpcListener{}
	port := newPort()
	logger := zap.NewNop()
